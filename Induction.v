...
Proof.
  ...
  - rewrite <- plus_n_Sm. rewrite IHm. reflexivity.
Qed.
...
Proof.
  ...
  - symmetry. exact (add_assoc m p n).
  - exact (add_comm p n).
Qed.
...
Proof.
  intros m n.
  induction m.
  - simpl. symmetry. exact (mul_0_r n).
  - simpl. rewrite IHm. rewrite mul_n_Sm. reflexivity.
Qed.
...
Theorem all3_spec : forall b c : bool,
  orb
    ...
    (orb (negb b)
         (negb c))
  = true.
Proof. destruct b; destruct c; reflexivity. Qed.
...
Example double_bin_five : double_bin (B1 (B0 (B1 Z))) = B0 (B1 (B0 (B1 Z))).
...
Example normalize_leading_B0 : normalize (B1 (B0 (B1 (B0 (B0 (B0 Z)))))) = B1 (B0 (B1 Z)).
Proof. reflexivity. Qed.
...
Theorem nat_to_bin_pres_incr : forall n, nat_to_bin (S n) = incr (nat_to_bin n).
Proof. reflexivity. Qed.

Theorem nat_to_bin_double : forall n,
...
Theorem add_rotate : forall n m k,
  n + m + k = k + n + m.
Proof.
  intros n m k.
  rewrite <- add_assoc.
