...
Proof.
  intros n hn.
  do 2 (apply evSS_ev in hn).
  exact hn.
Qed.

Theorem SSSSev__even__inversion : forall n,
  ev (S (S (S (S n)))) -> ev n.
Proof.
  intros n hn.
  inversion hn.
...
Proof.
  ...
  - apply ev_sum; trivial.
  - replace (n + m + (n + p)) with (double n + (m + p)) in hsum.
    + apply ev_ev__ev with (n := double n) in hsum.
      * exact hsum.
      * apply ev_double.
    + rewrite double_plus.
      rewrite add_assoc.
...
Proof.
  intros.
  induction H; simpl; simpl in H0; intro h.
  - destruct h; tauto.
  - destruct h; tauto.
  - tauto.
Qed.
...
Proof.
  intro h.
  set (hc := Perm3_In _ 4 [1; 2; 4] [1; 2; 3]).
  apply Perm3_symm in h.
  apply hc in h.
  - simpl in h.
    destruct h as [h1 | [h2 | [h3 | h4]]]; try discriminate.
    exact h4.
...
Theorem le_O_eq_0 : forall n, n <= 0 -> n = 0.
Proof.
  intros n.
...
Proof.
  intro n.
  induction n; intros m p q hnmpq.
  - left. apply O_le_n.
  - destruct p.
    + simpl in hnmpq.
      assert (IHn' := IHn (S m) 0 q).
      simpl in IHn'.
      replace (n + S m) with (S (n + m)) in IHn'.
      * specialize (IHn' hnmpq).
        destruct IHn'.
        ** apply le_O_eq_0 in H.
           rewrite H in hnmpq.
           right.
        ...
        ** right.
           apply Sn_le_m__n_le_m.
           exact H.
      * replace (S (n + m)) with (1 + (n + m)) by reflexivity.
        rewrite add_assoc.
        do 2 rewrite add_rotate.
        rewrite <- add_assoc.
        rewrite (add_comm m 1).
        reflexivity.
    + simpl in hnmpq.
      apply Sn_le_Sm__n_le_m in hnmpq.
      specialize (IHn _ _ _ hnmpq).
      destruct IHn.
...
Proof.
  intros.
  induction p.
  - rewrite add_0_r. exact H.
  - replace (m + S p) with (m + (1 + p)) by reflexivity.
    rewrite add_assoc.
    rewrite add_rotate.
    rewrite add_comm.
...
Theorem le_S_n_le_m : forall n m,
  n <= m -> n <> m -> S n <= m.
Proof.
  intros n m hnm.
  induction hnm; intros.
  - tauto.
...
Proof.
  ...
  unfold lt in *.
  split.
  - pose proof (L := plus_le (S n1) n2 m H).
    exact (proj1 _ _ L).
  - replace (S (n1 + n2)) with (n1 + S n2) in H.
    pose proof (L := plus_le n1 (S n2) m H).
    exact (proj2 _ _ L).
    + replace (S (n1 + n2)) with (1 + (n1 + n2)) by reflexivity.
      replace (n1 + (S n2)) with (n1 + (1 + n2)) by reflexivity.
      rewrite add_assoc.
      rewrite add_rotate.
      rewrite add_comm.
      rewrite (add_comm n2).
...
Proof.
  ...
  - intros m hnm.
    simpl.
    destruct m.
    + simpl in hnm. discriminate.
    + simpl in hnm. apply IHn. exact hnm.
Qed.
...
Proof.
  intros.
  exact (conj (leb_complete _ _) (leb_correct _ _)).
Qed.
...
Proof.
  ...
  induction h.
    ...
    + rewrite <- IHh. unfold fR.
      rewrite add_comm. simpl. rewrite add_comm.
      reflexivity.
    + simpl in IHh. injection IHh as IHh.
      rewrite add_comm in IHh. simpl in IHh.
      injection IHh as IHh.
      rewrite add_comm in IHh.
      exact IHh.
    + rewrite add_comm. exact IHh.
Qed.

Theorem fR_to_R : forall m n o,
  fR m n = o -> R m n o.
Proof.
  intros.
  destruct H.
...
Proof.
  intros.
  exact (conj (R_to_fR _ _ _) (fR_to_R _ _ _)).
Qed.
...
Inductive subseq : list nat -> list nat -> Prop :=
  | s_nil l: subseq nil l
  | s_skip (n : nat) l1 l2 (H : subseq l1 l2) : subseq l1 (n :: l2)
  | s_cons (n : nat) l1 l2 (H : subseq l1 l2) : subseq (n :: l1) (n :: l2)
.
...
Theorem app_not_nil {X : Type} : forall (head : X) (l1 l2 : list X),
  l1 ++ (head :: l2) <> [].
Proof.
  intros.
...
Proof.
  ...
  induction H.
  - apply s_nil.
  - replace (n :: l2) with ([n] ++ l2) by reflexivity.
    rewrite <- app_assoc.
    apply s_skip.
    exact IHsubseq.
  - replace (n :: l2) with ([n] ++ l2) by reflexivity.
...
Proof.
  ...
  - inversion h12. apply s_nil.
  - apply IHh23 in h12.
    apply s_skip.
    exact h12.
  - inversion h12; subst.
    + apply s_nil.
    + apply IHh23 in H1. apply s_skip. exact H1.
    + apply s_cons. apply IHh23. apply H1.
Qed.
(** [] *)
...
Theorem EmptyStr'_EmptyStr {T : Type} : forall ss,
  ss =~ @EmptyStr T <-> ss =~ @EmptyStr' T.
Proof.
  intros.
  split; intro.
  - inversion H. apply MStar0.
  - inversion H.
    + apply MEmpty.
    + inversion H2.
Qed.
...
Proof.
  ...
  split; intro h.
  - destruct h as [s hs].
    induction hs; simpl; try reflexivity.
    + apply andb_true_iff. apply (conj IHhs1 IHhs2).
    + apply orb_true_iff. left. exact IHhs.
    + apply orb_true_iff. right. exact IHhs.
  - induction re; simpl in h.
    + discriminate.
    + exists []. apply MEmpty.
    + exists [t]. apply MChar.
    + rewrite andb_true_iff in h. destruct h as [h1 h2].
      specialize (IHre1 h1). destruct IHre1 as [f hf].
      specialize (IHre2 h2). destruct IHre2 as [s hs].
      exists (f ++ s).
      apply MApp; trivial.
    + rewrite orb_true_iff in h.
      destruct h as [h | h].
      * specialize (IHre1 h).
        destruct IHre1 as [s hs].
        exists s.
        apply MUnionL.
        exact hs.
      * specialize (IHre2 h).
        destruct IHre2 as [s hs].
        exists s.
        apply MUnionR.
        exact hs.
    + exists []. apply MStar0.
...
Proof.
  ...
  induction H; try discriminate Eq; injection Eq as Eq; subst.
  - exists [].
    simpl.
    split.
    + reflexivity.
  ...
  - specialize (IHexp_match2 eq_refl).
    destruct IHexp_match2 as [ss' hss'].
    exists ([s1] ++ ss').
    ...
    + reflexivity.
    + intros s' hs'.
      destruct hs' as [hs' | hs']; subst.
      * exact H.
      * specialize (hss2 s' hs').
        exact hss2.
Qed.
(** [] *)
