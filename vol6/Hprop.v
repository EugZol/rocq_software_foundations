...
Proof using.
  ...
  - destruct H1. destruct H0. intuition.
    ...
    rewrite Fmap.union_empty_l. exact H0.
  - eexists. exists h.
    repeat split.
    + exact H1.
...
Proof using.
  ...
  {
    intros (h1&h'&M1&M2&D&U).
    destruct M2 as (h2&h3&M3&M4&D'&U').
    subst.
    rewrite Fmap.disjoint_union_eq_r in D.
    exists (h1 \u h2) h3.
    splits.
    - apply hstar_intro; auto.
    - assumption.
    - rewrite Fmap.disjoint_union_eq_l.
      split; apply Fmap.disjoint_sym; auto.
    - rewrite <- Fmap.union_assoc. reflexivity.
  }
Qed.
...
Proof using.
  ...
  split; intro H'.
  - destruct H' as (h2&h3&Ph2&Hh3&h2_h3_dis&h_h2_h3).
    destruct Ph2 as [H2 P']. subst.
    apply (conj P').
