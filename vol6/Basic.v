...
Lemma quadruple_verif : forall (n : int),
  triple (quadruple n)
    \[]
    (fun r => \[r = 4*n]).
Proof.
...
Lemma inplace_double_verif : forall (p : loc) (n : int),
  triple <{ inplace_double p }>
    (p ~~> n)
    (fun _ => (p ~~> (2*n))).
Proof.
  xwp. do 3 xapp. xsimpl. math.
Qed.
...
Lemma transfer_verif : forall (p q : loc) (n m : int),
  triple (transfer p q)
    (p ~~> n \* q ~~> m)
    (fun _ => p ~~> (m + n) \* q ~~> 0).
Proof using.
  intros. xwp. repeat xapp. xsimpl. math.
Qed.
...
Lemma transfer_verif_aliased : forall (p : loc) (n : int),
  triple (transfer p p)
    (p ~~> n)
    (fun _ => p ~~> 0).
Proof using.
...
Lemma triple_ref_greater_abstract : forall (p : loc) (n : int),
  triple (ref_greater p)
    (p ~~> n)
    (funloc q => \exists m, \[ m > n ] \* p ~~> n \* (q ~~> m)).
Proof using.
  intros. xapp triple_ref_greater. intro q.
  xsimpl; [reflexivity | math].
Qed.
...
Proof using.
  xwp.
  do 2 (xapp triple_rand; [math|]; intros).
  xapp. xapp. xsimpl; math.
Qed.
...
Proof using.
  intros p n m.
  gen p n.
  destruct (m >=? 0) eqn:Hm; intros p n.
  - assert (m >= 0) as Hm'.
    { rewrite Z.geb_le in Hm. math. }
    rewrite max_r by math.
    xapp triple_repeat_incr.
    + math.
    + xsimpl.
  - assert (m < 0) as Hm'.
    { rewrite Z.geb_leb in Hm. rewrite Z.leb_gt in Hm. math. }
    xwp. xapp. xif. { math. }
    intros _.
    xval. xsimpl.
    rewrite max_l by math.
    math.
Qed.
...
Proof using.
  ...
  induction_wf IH: (downto 0) m. unfold downto in IH.
  intros.
  destruct (m =? 0)%Z eqn:Hmz.
  {
    rewrite Z.eqb_eq in Hmz.
    subst.
    xwp. repeat xapp. xif. { math. } intros _.
    xval. xsimpl. math.
  }
  assert (m > 0) as Hmz'. { rewrite Z.eqb_neq in Hmz. math. }
  xwp. repeat xapp.
  xif; [|math].
  intros _.
  xapp. xapp. xapp; [math|math|].
  xsimpl. math.
Qed.
...
Proof using.
  ...
  xwp.
  xapp.
  destruct (i =? n)%Z eqn:Hieqn.
  {
    rewrite Z.eqb_eq in Hieqn.
    subst.
    xif; [math|].
    intros _.
    xval. xsimpl.
  }
  assert (i < n) as Hin. { rewrite Z.eqb_neq in Hieqn. math. }
  xif; [|math]. intros _.
  do 4 xapp.
  rewrite Z.mul_comm. rewrite <- facto_succ by math.
  xapp; [math | math | xsimpl].
Qed.
...
Proof using.
  introv. induction_wf IH: (upto n) i. introv Hn.
  destruct Hn as [(?&?)|?].
  { xapp triple_factoimp_aux_zero; [math | math | xsimpl]. }
  xapp triple_factoimp_aux; [math | xsimpl].
Qed.
...
Proof using.
  ...
  xapp triple_factoimp_aux'.
  {
    destruct (n =? 0)%Z eqn:Hnz.
    * rewrite Z.eqb_eq in Hnz. left. math.
    * rewrite Z.eqb_neq in Hnz. right. math.
  }
  { rewrite facto_init; math. }
